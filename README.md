...
* 新增知识库向量化能力，支持文档切片、Embedding、LlamaIndex 编排与 Milvus 向量检索。
...
* `module_knowledge`：知识库文档管理、企业/部门/个人隔离、LlamaIndex + Milvus 检索能力。
...
* `REDIS_*`：Redis 连接配置。
* `JWT_*`：登录认证与令牌配置。
* `KNOWLEDGE_*`：知识库 Provider、Milvus、Embedding、切片相关配置。
...
* 知识库能力：`llama-index-core`、`llama-index-vector-stores-milvus`、`pymilvus`、`python-docx`、`pypdf`

说明：

* 如果只是基础后台运行，安装 `requirements.txt` 即可。
* 如果需要使用知识库真实索引能力，必须确保 `llama-index-core`、`llama-index-vector-stores-milvus`、`pymilvus`、`python-docx`、`pypdf` 已正确安装。
* 如果运行环境切换了虚拟环境，需要确认 `pip install` 和 `python3 app.py` 使用的是同一个解释器。

### 知识库框架说明

当前默认知识库 Provider 已切换为 `llamaindex`，底层向量库仍使用 `Milvus`。

后端默认配置如下：

* `KNOWLEDGE_PROVIDER=llamaindex`
* `KNOWLEDGE_LLAMAINDEX_MILVUS_COLLECTION=ruoyi_knowledge_document_llamaindex`

兼容说明：

* 旧版文档如果是基于原 `milvus` Provider 建立的索引，代码仍可兼容检索。
* 如果希望把历史文档统一迁移到 `llamaindex` 流程，进入“知识库”页面后对历史文档执行一次“重新索引”即可。

### 知识库任务与迁移说明

当前知识库索引任务不再依赖进程内 `BackgroundTasks`，而是改为基于 `Redis Stream` 的异步消费模式。

后端相关配置项包括：

* `KNOWLEDGE_TASK_STREAM_KEY`：知识库索引任务 Stream Key
* `KNOWLEDGE_TASK_STREAM_GROUP`：知识库索引消费组名称
* `KNOWLEDGE_TASK_STREAM_CONSUMER_PREFIX`：消费者名前缀
* `KNOWLEDGE_TASK_STREAM_BATCH_SIZE`：单次拉取任务数
* `KNOWLEDGE_TASK_STREAM_BLOCK_MS`：阻塞拉取等待时间
* `KNOWLEDGE_TASK_STREAM_CLAIM_IDLE_MS`：挂起任务自动认领的最小空闲时间

行为说明：

* 上传文档后，系统会先落库，再把索引任务写入 Redis Stream，由后台消费者异步执行 LlamaIndex 建索引。
* 对历史文档执行“重新索引”时，会按当前切片参数和默认 Provider 重新建立索引，并更新文档的切片配置。
* 文档索引归属现在使用数据库字段 `provider_name` 记录，不再只依赖 `remark` 中的弱结构化信息。
* 对老库启动时，系统会自动检测 `knowledge_document` 是否缺少 `provider_name` 字段，缺少时自动补齐。

删除与补偿说明：

* 删除文档时，会先尝试清理对应 Provider 的索引，索引清理成功后才删除数据库记录，再删除源文件。
* 如果索引清理失败，数据库记录会被保留，文档状态会被标记为 `error`，并写入错误信息，便于后续重试删除或补偿处理。
* 如果重新索引任务入队失败，文档同样会被标记为 `error`，并保留失败原因。

部署建议：

* 生产环境需要保证 Redis 可用，否则知识库上传与重新索引只能完成文件落库，无法进入异步索引阶段。
* 当前任务队列实现基于 Redis Stream，暂未引入 Celery；如果后续需要更复杂的任务编排、监控面板或多级重试策略，可再演进到 Celery。
