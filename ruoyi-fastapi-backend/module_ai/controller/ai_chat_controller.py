...
async def send_chat_message(
    ...
    chat_stream = AiChatService.chat_services(query_db, chat_req, current_user)
...
@Log(title='AI对话会话管理', business_type=BusinessType.UPDATE)
async def update_chat_session_title(
    request: Request,
    session_id: Annotated[str, Path(description='会话ID')],
    session_info: AiChatSessionTitleUpdateModel,
    query_db: Annotated[AsyncSession, DBSessionDependency()],
    current_user: Annotated[CurrentUserModel, CurrentUserDependency()],
) -> Response:
    result = await AiChatService.update_chat_session_title_services(
        query_db,
