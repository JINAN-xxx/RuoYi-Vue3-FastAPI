...
class AiChatRequestModel(BaseModel):
    ...
    temperature: float | None = Field(default=None, description='本次对话温度')
    ...
    use_knowledge: bool | None = Field(default=False, description='是否启用知识库')
    knowledge_document_ids: list[int] | None = Field(default=None, description='指定知识库文档ID列表')
...
class AiChatSessionTitleUpdateModel(BaseModel):
    """
    AI对话会话标题更新模型
    """
    ...
    session_title: str = Field(description='会话标题', min_length=1, max_length=80)
